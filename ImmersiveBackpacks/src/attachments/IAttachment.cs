...
///   * behavior — <see cref="OnAttached"/>/<see cref="OnDetached"/> fire on live hosts only (see below).
///
...
///
/// LIVE vs VALUE hosts: a node hosted by a BlockEntity or entity behavior is a long-lived instance and gets
...
public interface IAttachment
{
    ...
    /// whole node (including nested children) round-trips through it.</summary>
    ItemStack Stack { get; }

    /// <summary>
    /// Content fingerprint that render caches key on, folding this node's stack AND its children recursively.
    /// Changing a nested tool bubbles up to the root's hash so held/worn caches miss and the placed renderer's
    /// key changes. On value hosts this is the only invalidation signal; live hosts also get the push via
    /// <see cref="IAttachmentHost.OnAttachmentInvalidated"/>.
    /// </summary>
    int ContentHash { get; }
    ...
    /// Not called on value (ItemStack) hosts — those reconstruct per render, so keep behavior tree-derived.</summary>
...
/// <summary>
/// The live owner of a node pushes invalidation to (a BlockEntity or entity behavior). The only method is a
/// coarse "something about this attachment changed, re-derive content" — the host reacts by re-composing the
/// model, recomputing derived state (e.g., emitted light) and marking itself dirty for save and sync. It is
